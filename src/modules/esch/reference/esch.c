...
void esch(char *msg_start, char *msg_end, char *output_ptr) {
  uint32_t state[STATE_WORDS];
  size_t insize = (size_t) (msg_end - msg_start);
  
  Initialize(state);
  ProcessMessage(state, msg_start, insize);
  Finalize(state, output_ptr);
}
