...
class Esch_t:
    ...
    def try_load_esch_library(self):
        if Path("../shared/sparkle_suite.so").exists():
            ...
            sys.stdout.write("Jasmin Sparkle Suite library (.so) not found. Please compile it.\nExiting\n")
            exit()

    def load_esch_library(self):
        #try:
        self.jasmin_esch_dll = ctypes.cdll.LoadLibrary("../shared/sparkle_suite.so")

        args = [ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char), ctypes.POINTER(ctypes.c_char)]

        self.jasmin_esch_dll.esch_export.argtypes = args
        self.jasmin_esch_dll.esch_export.restype = None
    ...
    def try_load_esch_reference_library(self):
        if Path("reference/esch/src/esch.so").exists():
    ...
    def load_esch_reference_library(self):
        try:
            self.reference_esch_dll = ctypes.cdll.LoadLibrary("reference/esch/src/esch.so")
    ...
    def esch_jasmin(self, start_ptr, end_ptr, output_ptr):
        ...
        self.jasmin_esch_dll.esch_export(start_ptr, end_ptr, output_ptr)
